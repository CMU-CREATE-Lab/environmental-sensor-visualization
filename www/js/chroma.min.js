	...
	ColorScale = function() {
		...
		ColorScale.prototype.classifyValue = function(value) {
			...
			if (domain.length > 2) {
				n = domain.length - 1;
				i = me.getClass(value);
				minc = domain[0] + (domain[1] - domain[0]) * (0 + me._spread * .5);
				maxc = domain[n - 1] + (domain[n] - domain[n - 1]) * (1 - me._spread * .5);
				val = me._min + (domain[i] + (domain[i + 1] - domain[i]) * .5 - minc) / (maxc - minc) * (me._max - me._min)
			}
			return val
		};
